...
sample = [np.zeros((200,200,3), dtype=np.int32)] # have to pass as array for pass by reference!
...
sample = sample[0].astype(np.int32) # 32 bit ints even when running on ubuntu
if SAVE:
    np.save("./test/test_sample.npy", sample) # to save array after modifications to code
else:
    chk_sample = np.load("./test/test_sample.npy")
    chk_diff = (np.abs(sample - chk_sample) > 5).sum()
    print("diff = {}".format(chk_diff))
    assert(chk_diff < 1_000) # less than 1,000 out of 120,000 pixel components out by > 5

...
        self._action_debounce_interval = 0.3 # Seconds - ADJUST THIS VALUE (e.g., 0.2 to 0.5)
        # --- END DEBOUNCE TIMER ---
...
    """Due to pi3d not reliably detecting touch ...
    any pointer movement is considered a click event.
    ADDED: Debounce check to prevent rapid clicks from movement.
    """
    # <<< Keep this part - it updates self.__pointer_position >>>
    # self.__timestamp = time.time() # Already done in check_input
    # self.__update_pointer_position() # Already done in check_input

    # <<< Use the __pointer_moved check from original code >>>
    if self.__pointer_moved():
        current_time = self.__timestamp # Use timestamp from check_input

        # --- ACTION DEBOUNCE CHECK ---
        if current_time - self._last_action_time > self._action_debounce_interval:

            # --- Original Logic based on movement detection ---
            if not self.controller.display_is_on:
                self.controller.display_is_on = True
                self._last_action_time = current_time # Update time even for display on
            elif self.__pointer_position[1] < self.__viewer.display_height // 2 - self.__menu_height:
                # Touch in main area
                if self.menu_is_on:
                    self.menu_is_on = False
                    self._last_action_time = current_time # Update time
                else:
                    self.__handle_click()
                    self._last_action_time = current_time # Update time
            else:
...
                    self.__handle_click()
                    self._last_action_time = current_time # Update time
                # Reset clock for autohide (or show menu if it was off)
                self.menu_is_on = True
                # self._last_action_time = current_time # Decide if showing menu counts as action

        else:
             logger.debug("Touch movement detected, but action debounced (too fast).")
        # --- END DEBOUNCE CHECK ---
